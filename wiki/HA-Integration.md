...
3. **Auto-ACK dla `TYPE=s`**: ACK zawsze, natychmiast
4. **CRC = `crc81wire`**: licz i porównuj (pole CRC w ramce to dziesiętne ASCII)
...
Gdy dostajesz `TYPE=s`, wysyłasz ACK:
...
> Nazewnictwo: `TYPE` bywa opisywane jako `TOP`, a `ID` jako `PKT`.

## Mapowanie do encji HA (praktyczne)

To jest “sensowny default”, który działa w większości instalacji:

- `I.<n>` (TYPE=s, przychodzi z modułu) → `binary_sensor` lub `sensor`
  - stan z `VAL` (często `0/1`)
- `O.<n>` (stan najlepiej brać z ACK: TYPE=a) → `switch` (albo `light`, jeśli wiesz że to światło)
  - stan z `VAL` (`0/1`)
- `UD` (ACK TYPE=a) → `cover`
...
- `(SRC, CMD, ID)` w oknie np. 20 sekund (dostosuj do instalacji)

## Testy end-to-end (szybkie)

1. **Czy integracja widzi ramki?** (sniffer w toolboxie)
2. **Czy integracja wysyła ACK na `TYPE=s`?** (w logach powinno być widać TX)
